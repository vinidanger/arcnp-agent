...
class ProcessRunner
{
    ...
    /**
     * Lê o access.log inteiro (não só as últimas N linhas — usado pelo
     * analytics de tráfego, que precisa contar o dia inteiro). Timeout
     * maior que tailNginxLog() de propósito: um log de conta grande
     * pode levar mais que os 30s padrão só pra ler do disco.
     */
    public function readFullAccessLog(string $domain): string
    {
        $result = Process::timeout(60)->run([
            'sudo', '-n', base_path('scripts/tail-log.sh'), 'access-full', $domain,
        ]);
